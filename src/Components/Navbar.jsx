...
const Navbar = () => {
  const { user, logoutUser } = useContext(Context); // use logoutUser from context
  ...
  return (
    <nav className="w-full bg-white shadow-md px-6 py-3 flex items-center justify-between relative z-50">
      {/* Logo */}
      <Link to="/" className="text-4xl font-bold text-green-600">NutriTech</Link>
      ...
      {menuOpen && (
        <>
          <div className="fixed inset-0 bg-black bg-opacity-30 z-30 lg:hidden" onClick={() => setMenuOpen(false)}></div>
          <div className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg transform transition-transform duration-300 z-40 ${menuOpen ? "translate-x-0" : "translate-x-full"} lg:hidden`}>
